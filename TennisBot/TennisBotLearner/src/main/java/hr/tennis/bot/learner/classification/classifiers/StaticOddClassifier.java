...
public class StaticOddClassifier extends Classifier {

	private static final long serialVersionUID = 1L;
	private List<double[]> maxMinList;

	public StaticOddClassifier(List<double[]> maxMinList) {
	...
	public void classify(double[] input, int dimensions, double[] output) {
		...
		if(input[0]>=1.2 && input[0]<=1.3 && input[1]>4.2){
			output[0]=1;
			output[1]=0;
		}else{
			output[0]=0;
	...
	public static void main(String[] args) throws IOException {
		...
		StaticOddClassifier staticClassifier = new StaticOddClassifier(maxMinList);
